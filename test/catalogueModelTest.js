...
describe('Test catalogue model ', function(){
    ...
    describe('In an empty database',function(){
        ...
        it('should saves and then contains the workshop with the title "The Ball Flow Game"', function () {
    ...
    describe('In a populated database',function(){
        ...
        it('should get the two workshops',function(){
            return catalogue.getWorkshops().then(function (results) {
                expect(results).not.to.be.undefined;
                expect(results).to.have.lengthOf(2);
                results.forEach(function (result) {
                    expect(result.title).to.be.oneOf(["Paper Plane Factory","Paper Plane Factory 2"]);
                    expect(result.duration).to.be.equal(60);
                    expect(result.content.steps).to.have.lengthOf(11);
                });
            },function (error) {
                isEnvironmentOk = false;
                expect.fail();
        ...
        it('should get the workshop by id',function(){
            if(isEnvironmentOk){
                return catalogue.getWorkshops().then(function (results) {

                    var workshop = results[0];
                    return catalogue.getWorkshop(workshop._id).then(function (result) {
                        expect(result).not.to.be.undefined;
                        expect(JSON.stringify(result._id)).to.be.equal(JSON.stringify(workshop._id));
                    },function (error) {
                        expect.fail();
                    });
        ...
        it('should delete the specified workshop', function () {
            if(isEnvironmentOk){
                return catalogue.getWorkshops().then(function (results) {

                    var workshop = results[0];
                    return catalogue.deleteWorkshop(workshop._id).then(function (result) {
                        expect(result).not.to.be.undefined;
                        expect(JSON.stringify(result._id)).to.equal(JSON.stringify(workshop._id));

                        return catalogue.getWorkshops().then(function (newResults) {
